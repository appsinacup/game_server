  ...
  if @webrtc_available do
    describe "webrtc:offer" do
      test "accepts a valid SDP offer and returns ok", %{socket: socket} do
        ...
        assert_push "webrtc:answer", answer_payload
        assert is_binary(answer_payload.sdp)
        assert answer_payload.type in ["answer", :answer]
      ...
      test "re-offer replaces existing peer", %{socket: socket} do
        ...
        assert_push "webrtc:answer", _answer1
        ...
        assert_push "webrtc:answer", _answer2
    ...
    describe "webrtc:ice" do
      ...
      test "accepts ICE candidate after offer", %{socket: socket} do
        ...
        ref = push(socket, "webrtc:offer", offer_json)
        assert_reply ref, :ok, %{}
        assert_push "webrtc:answer", _answer
    ...
    describe "WebRTC not available" do
      test "webrtc:offer returns unknown_event when deps not installed", %{socket: socket} do
        ref = push(socket, "webrtc:offer", %{"sdp" => "fake", "type" => "offer"})
        assert_reply ref, :error, %{error: "unknown_event"}
      end

      test "webrtc:ice returns unknown_event when deps not installed", %{socket: socket} do
        ref = push(socket, "webrtc:ice", %{"candidate" => "fake"})
        assert_reply ref, :error, %{error: "unknown_event"}
      end

      test "webrtc:close returns unknown_event when deps not installed", %{socket: socket} do
        ref = push(socket, "webrtc:close", %{})
        assert_reply ref, :error, %{error: "unknown_event"}
      end
    end
