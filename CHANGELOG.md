...
- [added] **Unique usernames** — every user now has a unique `username` handle next to the non-unique `display_name`: lowercase, 3–32 chars of `a-z 0-9` with non-consecutive `. _ -` separators (`LIMIT_MIN_USERNAME`/`LIMIT_MAX_USERNAME`). The server generates one at registration — slugified OAuth name (`dragos-4821`) or a random word for email/device signups (`sheep-4821`; never derived from the email address) — and the new `before_user_register(user, attrs)` pipeline hook lets plugins override it before insert (invalid or taken hook values fall back to a generated name so login never breaks). Players change it via `PATCH /api/v1/me/username` (strict validation; gate or reject names in `before_user_update`, e.g. profanity). `username` is included in all user JSON; lookup with `Accounts.get_user_by_username/1`. The migration backfills existing users automatically.
