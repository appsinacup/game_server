defmodule GameServerWeb.Api.V1.UserController do
  ...
  operation(:index,
    ...
    parameters: [q: [in: :query, schema: %Schema{type: :string}], page: [in: :query, schema: %Schema{type: :integer}], page_size: [in: :query, schema: %Schema{type: :integer}]],
    responses: [ok: {"Users (paginated)", "application/json", %Schema{type: :object, properties: %{data: %Schema{type: :array, items: %Schema{type: :object, properties: %{id: %Schema{type: :integer}, email: %Schema{type: :string}, display_name: %Schema{type: :string}, profile_url: %Schema{type: :string}}}}, meta: %Schema{type: :object, properties: %{page: %Schema{type: :integer}, page_size: %Schema{type: :integer}, count: %Schema{type: :integer}, total_count: %Schema{type: :integer}, total_pages: %Schema{type: :integer}, has_more: %Schema{type: :boolean}}}}}}]
  )
  ...
  operation(:show,
    ...
    responses: [ok: {"User", "application/json", %Schema{type: :object, properties: %{id: %Schema{type: :integer}, email: %Schema{type: :string}, display_name: %Schema{type: :string}, profile_url: %Schema{type: :string}}}}, not_found: {"Not found", "application/json", nil}]
  )
  ...
  def index(conn, params) do
    ...
    json(conn, %{data: serialized, meta: %{page: page, page_size: page_size, count: count, total_count: total_count, total_pages: total_pages, has_more: count == page_size}})
  end
