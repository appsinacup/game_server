/**
 * GameWebRTC — Browser-side WebRTC DataChannel client for game_server.
 *
 * Uses an existing Phoenix channel (UserChannel) for SDP/ICE signaling.
 ...
 *   const userChannel = socket.channel("user:123", {})
 *   userChannel.join()
 *
 ...
 *     onChannelOpen:  (label)        => console.log("open", label),
 *     onChannelClose: (label)        => console.log("close", label),
 ...
 * - "state":  unreliable, unordered — high-frequency state sync
 */
...
export class GameWebRTC {
  /**
   * @param {Object} channel — A joined Phoenix channel (e.g. UserChannel)
   * @param {Object} opts
   * @param {Array}  opts.dataChannels — Array of {label, ordered, maxRetransmits?} defs
   * @param {Array}  opts.iceServers   — RTCIceServer configs
   * @param {Function} opts.onData         — (label, data: ArrayBuffer|string) => void
   * @param {Function} opts.onChannelOpen  — (label) => void
   * @param {Function} opts.onChannelClose — (label) => void
   ...
   * Creates an RTCPeerConnection, opens DataChannels, sends SDP offer
   ...
   * @param {string} label — channel label (e.g. "events", "state")
   * @param {string|ArrayBuffer|Blob} data — payload to send
   ...
   */
  isConnected() {
    return this.pc && this.connectionState === "connected"
  }
  ...
  _setupChannelListeners() {
    ...
    // Server notifies about connection state
    this._stateRef = this.channel.on("webrtc:state", (payload) => {
      const cb = this.opts.onStateChange
      if (cb) cb(payload.state)
    })

    // Server relays DataChannel data back (only used for server-initiated sends
    // through the WebSocket as fallback)
  ...
  _wireDataChannel(dc) {
    dc.onopen = () => {
      ...
      // Resolve connect() promise when FIRST channel opens
      if (this._resolveConnect) {
        this._resolveConnect()
        this._resolveConnect = null
        this._rejectConnect = null
