...
class ApiClient extends RefCounted:
	...
	func {{ $operation.OperationId | apiFuncName }}_async(
        ...
        {{- end }}
				{{- else }}
						{{- $sessionName := printf "%sSession" "{{.ClassName}}" }}
						{{- if hasDefinition $sessionName }}
						p_session : {{$sessionName}}
						{{- else }}
						p_session : Dictionary
						{{- end }}
        {{- end }}
	...
	{{- if not $parameter.Required }}{{/* Godot does not support typed optional parameters yet. */}}
		, {{ $argument }} = null # : {{ firstNonEmpty $parameter.Type $parameter.Schema.Type }}
		...
		, {{ $argument }} : {{ $parameter.Schema.Ref | cleanRef }}
            {{- end }}
        {{- else }}
		, {{ $argument }} : {{ godotType (firstNonEmpty $parameter.Type $parameter.Schema.Type) $parameter.Schema.Ref $parameter.Items.Type "" (isRefToEnum (cleanRef $parameter.Schema.Ref)) }}
        {{- end }}
	{{- end }}
	)
	...
	{{- end }}:
		...
		if {{ $argument }} != null:
            {{- end }}
				{{- if eq (firstNonEmpty $parameter.Type $parameter.Schema.Type) "integer" }}
			query_params += "{{- $snakecase }}=%d&" % {{ $argument }}
				{{- else if eq (firstNonEmpty $parameter.Type $parameter.Schema.Type) "string" }}
			query_params += "{{- $snakecase }}=%s&" % {{.ClassName}}Serializer.escape_http({{ $argument }})
				{{- else if eq (firstNonEmpty $parameter.Type $parameter.Schema.Type) "boolean" }}
			query_params += "{{- $snakecase }}=%s&" % str(bool({{ $argument }})).to_lower()
				{{- else if eq (firstNonEmpty $parameter.Type $parameter.Schema.Type) "array" }}
			for elem in {{ $argument }}:
				query_params += "{{- $snakecase }}=%s&" % elem
                {{- else }}
		# Unsupported or unknown query parameter type: {{ firstNonEmpty $parameter.Type $parameter.Schema.Type }}
                {{- end }}
            {{- end }}
            {{- end }}
...
func apiFuncName(input string) (output string) {
	// Operation IDs may already be in snake_case or camelCase — convert them
	// directly and do not strip characters from the front. The previous
	// implementation removed the first 7 characters which caused many names
	// to lose their first letters (eg. "request_async" became "equest_async").
	output = pascalToSnake(input)
	return
}
...
// firstNonEmpty returns the first non-empty string among the two inputs
...
func main() {
	...
	// If the spec uses OpenAPI v3 components.schemas instead of the
	// Swagger 2 top-level "definitions") then populate Definitions from
	// components.schemas so the rest of the generator (which expects
	// Definitions) works with either format.
	if len(schema.Definitions) == 0 && len(schema.Components.Schemas) > 0 {
	...
	fmap := template.FuncMap{
					"hasDefinition": func(name string) bool {
						...
						// Try camelCase / PascalCase variants
						if _, ok := schema.Definitions[pascalToCamel(name)]; ok {
							return true
						}
						if _, ok := schema.Definitions[camelToPascal(name)]; ok {
							return true
						}
						return false
					},
	...
	if err := tmpl.Execute(writer, schema); err != nil {
		fmt.Printf("Template execution error: %v\n", err)
		writer.Flush()
		os.Exit(1)
	}
	if err := writer.Flush(); err != nil {
		fmt.Printf("Failed flushing to output file: %v\n", err)
		os.Exit(1)
	}
}
